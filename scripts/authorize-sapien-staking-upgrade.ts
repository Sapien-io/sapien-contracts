...
export default async function main() {
  ...
  const sapienStaking = await ethers.getContractAt("SapienStaking", config.staking.proxyAddress)
  const SapienStakingV2 =  await ethers.getContractFactory("SapienStaking")

  const upgradedImpl= await upgrades.deployImplementation(SapienStakingV2, {
    kind: "uups" 
  })
  ...
  const tx: MetaTransactionData = {
    to: config.staking.proxyAddress,
    value: "0",
    data: callData,
  ...
  config.staking.authorizedUpgradedImplementation = upgradedImpl as `0x${string}`

  fs.writeFileSync(
    path.join(deployDir, "SapienStaking.json"),
    JSON.stringify(config.staking, null, 2)
  )
  return true
}

...
export default async function main() {
  ...
  const sapienStaking = await ethers.getContractAt("SapienRewards", config.rewards.proxyAddress)
  const SapienRewardsV2 =  await ethers.getContractFactory("SapienRewards")

  const upgradedImpl= await upgrades.deployImplementation(SapienRewardsV2, {
    kind: "uups" 
  })

  const callData = sapienStaking.interface.encodeFunctionData(
    "authorizeUpgrade",
    [upgradedImpl]
  )
  const tx: MetaTransactionData = {
    to: config.rewards.proxyAddress,
    value: "0",
    data: callData,
  ...
  config.rewards.authorizedUpgradedImplementation = upgradedImpl as `0x${string}`

  fs.writeFileSync(
    path.join(deployDir, "SapienRewards.json"),
    JSON.stringify(config.rewards, null, 2)
  )
  return true
}

...
- **Details**: Defines project parameters: `rewardToken`, `minStakeToClaim`, `minValidations`, `requiredSkill`, and an IPFS CID for the project specification.
...
- **Enqueueing**: `SapienCore` automatically calls `enqueueValidation` in `ValidationOracle`, which creates multiple validation slots (up to `maxValidations`) in a pending queue.
...
- **Trigger**: Anyone can call `finalizeContribution` in `SapienCore` once `minValidations` have been revealed and all active commits are either revealed or expired.
...
- **Validator Settlement**: Accurate validators receive a portion of the `validatorRewardBasisPoints` from the project rewards. Outliers are slashed.

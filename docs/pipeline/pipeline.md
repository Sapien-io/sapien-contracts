**Architecture**: Sapien PoQ v0.5 (v0.5) — QualityEngine + StakeVault + ConsensusLib. See `docs/v0.5-contracs.md`.
...
A1 — Intent & Spec Extractor Agent
...
Outputs:
	...
	•	critical_flows.md (deposit/withdraw/borrow/liquidate/etc.)
...
Outputs:
	•	surface.json: external/public functions, payable paths, external calls, delegatecalls
...
A5 — Fuzz & Invariant Runner Agent (Tool Agent)
...
Inputs: Critical flows, external calls, oracle usage, fee logic, rounding.
Outputs:
	•	findings_econ.json (sandwichability, oracle manipulation vectors, free options, griefing, rounding bias)
	•	mev_scenarios.md with step-by-step attacker playbooks

⸻

A7 — Upgradeability & Storage Safety Agent

Job: ensure upgrade paths won’t brick / get hijacked.
Skill File: `upgrade-storage-review-skill.md`
Inputs: proxy pattern, storage layout, initializer patterns, admin checks.
Outputs:
...
A8 — Permissions & Trust Risk Agent
...
Job: check assumptions about external protocols (ERC20, Oracles, Uniswap).
...
Run: A6
Gate: If econ finding is Critical+High-likelihood (e.g., oracle manipulation with clear path), FAIL.
...
{
  "id": "SEC-REENT-001",
  "title": "Reentrancy on withdraw() allows double-withdraw",
  "severity": "CRITICAL",
  "likelihood": "HIGH",
  "impact": "Loss of funds",
  "component": "Vault",
  "evidence": [
    {"file":"Vault.sol","lineStart":123,"lineEnd":180,"note":"External call before state update"},
    {"type":"trace","ref":"fuzz_results.json#case42"}
  ],
  "exploit_sketch": [
    "Attacker deposits",
    "Calls withdraw() via malicious receiver",

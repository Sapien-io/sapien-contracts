...
\title{\textbf{Sapien v2: A Decentralized Protocol for Verifiable, Consensus-Based Quality Signals in AI Workflows}}
...
\begin{enumerate}
    ...
    \item \textbf{Misaligned Incentives:} Participants in low-wage jurisdictions maximize throughput rather than accuracy, leading to "lazy labeling."
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Contributors:} Participants who perform the primary task (e.g., data annotation, output evaluation, chain-of-thought reasoning, safety assessment). They must stake tokens to claim work slots, providing "skin in the game."
    \item \textbf{Validators:} Independent reviewers who audit the work of Contributors. They reach consensus on quality scores.

    \textbf{Note on Participant Type:} Contributors and Validators may be human workers, AI agents, or hybrid teams. The protocol is agnostic to participant type. Task Originators may specify requirements (e.g., human-only) via the Task Definition Spec.
    \item \textbf{Oracles (Adapters):} Middleware that bridges offchain data (e.g., images stored in IPFS, CVAT interfaces) with onchain coordination.
\end{itemize}
...
Sapien utilizes \textbf{SqrtStakeConsensus}, which combats plutocracy (whale dominance) by weighing votes based on the square root of stake.
...
Where $S_{v_i}$ is the staked amount. The square root function $\sqrt{S_{v_i}}$ introduces \textit{quadratic voting} properties, diminishing the marginal influence of additional capital and reducing whale power by approximately 22\% compared to linear weighting.

    ...
    ReentrancyGuardUpgradeable
{
     ...
     */
    function calculateUserTotalPendingRewards(address user) 
        public 
        view 
     ...
     */
    function calculateUserVotingPower(address user) 
        public 
        view 
     ...
     */
    function calculateEstimatedAPY(uint256 lockUpPeriod) 
        public 
        view 
        returns (uint256 apy) 
    {
    ...
    function getUserRewardSummary(address user) 
        external 
        view 
        returns (RewardSummary memory summary) 
    {
        ...
        if (stakingVault.hasActiveStake(user)) {
            ...
            (
                ...
                ,
                bool isActive
            ) = stakingVault.getStakeDetails(user, 1);
            
            if (isActive && amount > 0) {
                // Calculate APY based on current multiplier
                summary.averageAPY = (baseRewardRate * SECONDS_PER_YEAR * multiplier * 10000) / (MULTIPLIER_PRECISION * 1e18);
            }
        }
    }

...
contract BatchRewardsTest is Test {
    ...
    function test_BatchRewards_PauseWithDifferentUsers() public {
        ...
        vm.prank(user);
        vm.expectRevert(abi.encodeWithSignature("RewardsContractPaused(address)", address(usdcRewards)));
        batchRewards.batchClaimRewards(REWARD_AMOUNT, sapienOrderId1, sapienSignature1, REWARD_AMOUNT / 10**12, usdcOrderId1, usdcSignature1);
        
        vm.prank(user2);
        vm.expectRevert(abi.encodeWithSignature("RewardsContractPaused(address)", address(usdcRewards)));
        batchRewards.batchClaimRewards(REWARD_AMOUNT, sapienOrderId2, sapienSignature2, REWARD_AMOUNT / 10**12, usdcOrderId2, usdcSignature2);
        
        vm.prank(user3);
        vm.expectRevert(abi.encodeWithSignature("RewardsContractPaused(address)", address(usdcRewards)));
        batchRewards.batchClaimRewards(REWARD_AMOUNT, sapienOrderId3, sapienSignature3, REWARD_AMOUNT / 10**12, usdcOrderId3, usdcSignature3);
    }
    ...
    function test_BatchRewards_PauseErrorMessageAccuracy() public {
        ...
        vm.prank(user);
        vm.expectRevert(abi.encodeWithSignature("RewardsContractPaused(address)", address(sapienRewards)));
        batchRewards.batchClaimRewards(REWARD_AMOUNT, sapienOrderId, sapienSignature, REWARD_AMOUNT / 10**12, usdcOrderId, usdcSignature);
    ...
    function test_BatchRewards_PauseOrderConsistency() public {
        ...
        vm.prank(user);
        vm.expectRevert(abi.encodeWithSignature("RewardsContractPaused(address)", address(sapienRewards)));
        batchRewards.batchClaimRewards(MAX_REWARD, sapienOrderId, sapienSignature, MAX_REWARD / 10**12, usdcOrderId, usdcSignature);
    }
    ...
    function test_BatchRewards_PauseGasEfficiency() public {
        ...
        gasStart = gasleft();
        vm.prank(testUser2);
        try batchRewards.batchClaimRewards(REWARD_AMOUNT, sapienOrderId, sapienSignature, REWARD_AMOUNT / 10**12, usdcOrderId, usdcSignature) {

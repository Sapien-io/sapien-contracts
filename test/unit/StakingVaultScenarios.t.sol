...
contract StakingVaultScenariosTest is Test {
    ...
    function test_Scenario_ProgressiveStaker() public {
        ...
        assertTrue(stakingVault.hasActiveStake(alice));
        ...
        ) = stakingVault.getUserStakingSummary(alice);
        ...
        vm.startPrank(alice);
        sapienToken.approve(address(stakingVault), MINIMUM_STAKE * 5);
        stakingVault.increaseAmount(MINIMUM_STAKE * 5);
        vm.stopPrank();

        assertEq(stakingVault.totalStaked(), MINIMUM_STAKE * 6);

        // Verify the stake was increased but lockup period stays the same
        (totalStaked,,,,,, effectiveLockUpPeriod,) = stakingVault.getUserStakingSummary(alice);
        ...
        vm.startPrank(alice);
        stakingVault.increaseLockup(60 days); // Extend by 60 days
        vm.stopPrank();

        // Verify the lockup was extended and multiplier updated
        (,,,,, effectiveMultiplier, effectiveLockUpPeriod,) = stakingVault.getUserStakingSummary(alice);
        ...
        vm.startPrank(alice);
        sapienToken.approve(address(stakingVault), MINIMUM_STAKE * 10);
        stakingVault.increaseAmount(MINIMUM_STAKE * 10);
        vm.stopPrank();

        // Final verification
        (totalStaked,,,,,,,) = stakingVault.getUserStakingSummary(alice);
    ...
    function test_Scenario_EmergencyLiquidator() public {
        ...
        stakingVault.stake(stakeAmount, LOCK_365_DAYS);
        vm.stopPrank();
        ...
        vm.prank(bob);
        stakingVault.instantUnstake(emergencyAmount);
        ...
        (uint256 totalStaked,,,,,,,) = stakingVault.getUserStakingSummary(bob);
        ...
        vm.prank(bob);
        stakingVault.initiateUnstake(remainingAmount);
        ...
        vm.prank(bob);
        stakingVault.unstake(remainingAmount);
        ...
        assertFalse(stakingVault.hasActiveStake(bob));
    }
    ...
    function test_Scenario_StrategicRebalancer() public {
        // Charlie starts with a conservative approach
        vm.startPrank(charlie);
        sapienToken.approve(address(stakingVault), MINIMUM_STAKE * 5);
        stakingVault.stake(MINIMUM_STAKE * 5, LOCK_30_DAYS);
        vm.stopPrank();

        assertEq(stakingVault.totalStaked(), MINIMUM_STAKE * 5);
        ...
        stakingVault.increaseLockup(70 days);
        vm.stopPrank();
        ...
        ) = stakingVault.getUserStakingSummary(charlie);
        ...
        vm.startPrank(charlie);
        sapienToken.approve(address(stakingVault), MINIMUM_STAKE * 15);
        stakingVault.increaseAmount(MINIMUM_STAKE * 15);

        // Extend lockup to maximum for best multiplier
        stakingVault.increaseLockup(275 days); // Current 60 days remaining + 275 = 335 days
        vm.stopPrank();

        // Final verification of Charlie's strategic position
        (totalStaked,,,,,, effectiveLockUpPeriod,) = stakingVault.getUserStakingSummary(charlie);
        ...
        vm.prank(charlie);
        stakingVault.initiateUnstake(partialAmount);
        ...
        vm.prank(charlie);
        stakingVault.unstake(partialAmount);

        assertEq(sapienToken.balanceOf(charlie), charlieBalanceBefore + partialAmount);

        // Verify remaining stake
        (totalStaked,,,,,,,) = stakingVault.getUserStakingSummary(charlie);
        assertEq(totalStaked, MINIMUM_STAKE * 20); // 30 - 10
        assertTrue(stakingVault.hasActiveStake(charlie));
    }
    ...
    function test_Scenario_WeightedAverageStaking() public {
        ...
        vm.startPrank(david);
        sapienToken.approve(address(stakingVault), MINIMUM_STAKE);
        stakingVault.stake(MINIMUM_STAKE, LOCK_30_DAYS); // 1000 tokens, 30 days
        vm.stopPrank();
        ...
        stakingVault.stake(MINIMUM_STAKE * 3, LOCK_180_DAYS); // 3000 tokens, 180 days
        vm.stopPrank();
        ...
        (uint256 totalStaked,,,,,, uint256 effectiveLockUpPeriod,) = stakingVault.getUserStakingSummary(david);
        ...
        stakingVault.stake(MINIMUM_STAKE * 2, LOCK_365_DAYS); // 2000 tokens, 365 days
        vm.stopPrank();
        ...
        (totalStaked,,,,,, effectiveLockUpPeriod,) = stakingVault.getUserStakingSummary(david);
    ...
    function test_Scenario_LockupExtensionBenefits() public {
        ...
        vm.startPrank(eve);
        sapienToken.approve(address(stakingVault), MINIMUM_STAKE * 10);
        stakingVault.stake(MINIMUM_STAKE * 10, LOCK_90_DAYS);
        vm.stopPrank();

        // Check initial multiplier
        (,,,,, uint256 initialMultiplier,,) = stakingVault.getUserStakingSummary(eve);

        // After 30 days, Eve extends her lockup to get a better multiplier
        vm.warp(block.timestamp + 30 days);

        vm.prank(eve);
        stakingVault.increaseLockup(275 days); // 60 remaining + 275 = 335 days

        // Check new multiplier
        (,,,,, uint256 newMultiplier, uint256 effectiveLockUpPeriod,) = stakingVault.getUserStakingSummary(eve);
        ...
        (uint256 totalStaked,,,,, uint256 finalMultiplier,,) = stakingVault.getUserStakingSummary(eve);
    ...
    function test_Scenario_MultiUserInteraction() public {
        ...
        vm.startPrank(frank);
        sapienToken.approve(address(stakingVault), MINIMUM_STAKE * 20);
        stakingVault.stake(MINIMUM_STAKE * 20, LOCK_180_DAYS);
        vm.stopPrank();

        vm.startPrank(grace);
        sapienToken.approve(address(stakingVault), MINIMUM_STAKE * 15);
        stakingVault.stake(MINIMUM_STAKE * 15, LOCK_90_DAYS);
        vm.stopPrank();

        // Verify total staked
        assertEq(stakingVault.totalStaked(), MINIMUM_STAKE * 35);

        // Both users have independent stakes
        assertTrue(stakingVault.hasActiveStake(frank));
        assertTrue(stakingVault.hasActiveStake(grace));

        // Frank extends his lockup
        vm.warp(block.timestamp + 60 days);

        vm.prank(frank);
        stakingVault.increaseLockup(245 days); // 120 remaining + 245 = 365 days

        // Grace's stake unlocks first
        vm.warp(block.timestamp + 31 days); // Total: 91 days

        // Grace can unstake, Frank cannot
        (, uint256 graceUnlocked,,,,,,) = stakingVault.getUserStakingSummary(grace);
        (, uint256 frankUnlocked,,,,,,) = stakingVault.getUserStakingSummary(frank);
        ...
        // Grace unstakes half
        vm.prank(grace);
        stakingVault.initiateUnstake(MINIMUM_STAKE * 7);
        ...
        vm.prank(grace);
        stakingVault.unstake(MINIMUM_STAKE * 7);

        assertEq(sapienToken.balanceOf(grace), graceBalanceBefore + MINIMUM_STAKE * 7);

        // Verify individual and total states
        (uint256 graceStaked,,,,,,,) = stakingVault.getUserStakingSummary(grace);
        (uint256 frankStaked,,,,,,,) = stakingVault.getUserStakingSummary(frank);

        assertEq(graceStaked, MINIMUM_STAKE * 8); // 15 - 7
        assertEq(frankStaked, MINIMUM_STAKE * 20);
        assertEq(stakingVault.totalStaked(), MINIMUM_STAKE * 28); // 35 - 7
    }

...
contract POQ_013_UpheldDisputeSlotLifecycleAndPayment is BaseTest {
    ...
    function test_POQ013B_UpheldDisputeOnRejectedPaymentOrdering() public {
        ...
        _claimAndCommit(validator1, projectId, idx, 2000, VALIDATOR_STAKE);
        _claimAndCommit(validator2, projectId, idx, 2000, VALIDATOR_STAKE);
        _claimAndCommit(validator3, projectId, idx, 2000, VALIDATOR_STAKE);
        _reveal(validator1, projectId, idx, 2000);
        _reveal(validator2, projectId, idx, 2000);
        _reveal(validator3, projectId, idx, 2000);
        engine.computeConsensus(projectId, idx);
